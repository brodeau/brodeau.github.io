...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namsbc_cpl in configuration namelist', lwp )
      ...
      IF( nn_components == jp_iam_sas ) THEN
         ...
         IF( l_vect_exchg_T ) THEN
            srcv(jpr_ocx1)%clgrid = 'T'        ! oce components given at T-point  #lola
            srcv(jpr_ocy1)%clgrid = 'T'        !           and           T-point
            !LB.
         ELSE
      ...
      IF( l_vect_exchg_T ) THEN
         ...
         DO jj = 2, jpjm1
            DO ji = fs_2, fs_jpim1   ! vector opt.
               ...
               ztmp2(ji,jj) = 0.5 * ( vtau(ji,jj) + vtau(ji  ,jj-1) )
            END DO
         END DO
         CALL lbc_lnk( ztmp1, 'T', -1. )   ;   CALL lbc_lnk( ztmp2, 'T', -1. )
         IF( ssnd(jps_otx1  )%laction )  CALL cpl_snd( jps_otx1  , isec, RESHAPE ( ztmp1, (/jpi,jpj,1/) ), info )
