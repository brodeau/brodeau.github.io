MODULE sbccpl
   ...
   LOGICAL, PARAMETER, PUBLIC :: l_vect_exchg_T = .TRUE.  !LB: set to .TRUE. if all exchanges of vector fields should be performed on T-points
...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: sbccpl.F90 9429 2018-03-26 14:42:05Z nicolasmartin $
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namsbc_cpl in configuration namelist', lwp )
      ...
      IF( nn_components == jp_iam_opa ) THEN    ! OPA coupled to SAS via OASIS: force received field by OPA (sent by SAS)
         ...
         IF( l_vect_exchg_T ) THEN
            srcv(jpr_otx1)%clgrid = 'T'        ! oce components given at T-point  #daisy
            srcv(jpr_oty1)%clgrid = 'T'        !           and           T-point
            ! Vectors: change of sign at north fold ONLY if on the local grid
...
! Currently no namelist entry to determine sending of multi-category ice fraction so use the thickness entry for now
      ...
      IF( nn_components == jp_iam_opa ) THEN
         ...
         !LB:
         IF( l_vect_exchg_T ) THEN
            ssnd(jps_ocx1)%clgrid  = 'T'        ! oce components given at U-point
            ssnd(jps_ocy1)%clgrid  = 'T'        !           and           V-point
            sn_snd_crt%clvgrd = 'T'
            !LB.
         ELSE
   ...
   SUBROUTINE sbc_cpl_rcv( kt, k_fsbc, k_ice )
      ...
      !    lolo opa-sas: srcv(jpr_otx1)%laction=True!        ! ========================= !
      IF( srcv(jpr_otx1)%laction ) THEN                      !  ocean stress components  !
         ...
         !  lolo opa-sas: nrcvinfo(jpr_otx1) , OASIS_Rcv = 1 , 1
         IF(  nrcvinfo(jpr_otx1) == OASIS_Rcv ) THEN
            !
            ! LOLO: dans notre cas opa-sas rien n'est fait comme modif dans ce bloque sur
            !       frcv(jpr_otx1)%z3(:,:,1) et frcv(jpr_oty2)%z3(:,:,1) !!
            !
            !   lolo opa-sas: sn_rcv_tau%clvref = 'spherical'
            ...
            ENDIF
            !
            !   lolo opa-sas: sn_rcv_tau%clvor = 'local grid'
         ...
         ELSE
            llnewtx = .FALSE.
         ENDIF !IF(  nrcvinfo(jpr_otx1) == OASIS_Rcv )
         !                                                   ! ========================= !
   ...
   SUBROUTINE sbc_cpl_snd( kt )
      ...
      IF( ssnd(jps_ocx1)%laction ) THEN                      !      Surface current      !
         ...
         ELSE
            ...
            CALL lbc_lnk( zotx1, ssnd(jps_ocx1)%clgrid, -1. )   ;   CALL lbc_lnk( zoty1, ssnd(jps_ocy1)%clgrid, -1. )
            !
         ENDIF !IF( nn_components == jp_iam_opa )
         !
         ! lolo opa-sas mode : sn_snd_crt%clvor = 'local grid'
         ...
         ENDIF
         !
         ! lolo opa-sas mode :  sn_snd_crt%clvref = 'spherical'
         ...
         !
      ENDIF  !IF( ssnd(jps_ocx1)%laction )
      !
      !
